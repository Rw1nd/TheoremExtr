...
def insert_to_parser(pjsonpath, rjsonpath, PROJNAME):
    ...
    for x in datap:
        ...
        for d in range(len(decl)):
            ...
            if kind == "VernacStartTheoremProof":
                pobj = decl[d]
    ...
    f = open("./data_json/" + outname + ".json", 'w')
...
def getdetail(PROJNAME):
    ...
    f = open("./data_json/" + detailname + ".json")
    data = json.load(f)
    f.close()
    ...
    for x in data:
        ...
        newdict[lemmaname] = {"linenum":linenum, "usedfunction" : usedfunction, "indnamelist":indnamelist, "fullpath": fullpath}
    ...
    f = open('./detail/' + outname + '-detail.json', 'w')
...
def jsontotxt(PROJNAME):
    ...
    f = open('./data_json/' + textname + '.json', 'r')
    data = json.load(f)
    f.close()
...
def start_ext(PROJNAME):
    ...
    data_json_path = "/home/opam/data/data_json/"
    detail_path = "/home/opam/data/detail/"
    text_path = "/home/opam/data/text/"

    init_dir(JSONFILE_PATH, env_json_path, detail_path, data_json_path, parser_json_path, text_path)

    build_project(benchmark_path, "/home/opam/data/", PROJNAME)

    merge_all(JSONFILE_PATH, PROJNAME)

    run_subprocess("coqc", os.path.join("./headers", PROJNAME.replace("-", "_")+".v"))
    shutil.move(os.path.join(env_json_path, PROJNAME.replace("-", "_")+".json"), os.path.join(env_json_path, PROJNAME.replace("-", "_")+"_env.json"))
